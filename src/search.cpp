...
namespace Search {
    ...
    void initArr() {
        ...
        for (int depth = 1; depth < 9; depth++) {
            LMPTable[0][depth] = 3 + 2 * depth * depth / 5;
    ...
    for (rdepth = e.rdepth; rdepth <= e.limits.depth; rdepth = e.rdepth) {
        ...
        if (thread_id == 0 && e.use_time) {
            ...
            if (currtime - e.start_time >= ((e.time_limit_max - e.start_time) * 7) / 10) {
                if (last_score <= e.rootbestmove.s - 20)
...
int search_t::search(bool inRoot, bool inPv, int alpha, int beta, const int depth, const int ply, const bool inCheck) {
    ...
    uint16_t cm = (lm.m != 0) ? countermove[pos.side][pos.getPiece(lm.moveTo())][lm.moveTo()] : 0;
    ...
    for (move_t m; mp.getMoves(m, skipquiets);) {
        ...
        if (best_score == NOVALUE) {
            ...
            if (!inPv && !inCheck && !moveGivesCheck && depth < 9) {
                if (!isTactical && futilityMargin <= alpha) { skipquiets = true; continue; }
                if (!isTactical && movestried >= LMPTable[improving][depth]) { skipquiets = true; continue; }
                if ((!isTactical || mp.stage == STG_BADTACTICS) && !pos.staticExchangeEval(m, isTactical ? -100 * depth : -10 * depth * depth)) continue;
            }
