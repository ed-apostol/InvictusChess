...
void abdada_table_t::setBusy(const uint32_t hash) {
	...
	for (int t = 4; t--; ++entry) {
		...
		if (entry->movehash == hash) return;
	}
	replace->movehash = hash;
}

void abdada_table_t::resetBusy(const uint32_t hash) {
	movehash_bucket_t *B = getEntry(hash);
	move_hash_t *entry = &B->bucket[0];
	for (int t = 4; t--; ++entry) {
		if (entry->movehash == hash) entry->movehash = 0;
	}
}

bool abdada_table_t::isBusy(const uint32_t hash) {
	movehash_bucket_t *B = getEntry(hash);
	move_hash_t *entry = &B->bucket[0];
	for (int t = 4; t--; ++entry) {
		if (entry->movehash == hash) return true;
	}
	return false;
}